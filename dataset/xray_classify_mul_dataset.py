...
__all__ = ['XrayClassifyMulClsDataset']


class XrayClassifyMulClsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        foreign_label = self.metas[index][1]
        if foreign_label == 0:
            foreign_label = [1, 0]
        else:
            foreign_label = [0, 1]

        position_label = self.metas[index][3]
        if position_label == 1:
            position_label = [1, 0, 0]
        elif position_label == 2:
            position_label = [0, 1, 0]
        else:
            position_label = [0, 0, 1]

        foreign_label = torch.FloatTensor(foreign_label)
        position_label = torch.FloatTensor(position_label)

        return img, foreign_label, position_label
