...
def visualize_in_evaluate(input, landmarks, pixelspacing):
    ...
    for idx, (y_pos, x_pos) in enumerate(landmarks):
            ...
            if len(landmarks)==5 and (idx==2 or idx==4):
                ...
                if idx == 2:
                    continue
                if idx == 4:
                    cv2.circle(img, (lat_proximal[1], lat_proximal[0]), 5, ldm_color, -1)
                    cv2.putText(img, ldm_name_list[idx], (lat_proximal[1]+10, lat_proximal[0]-10), cv2.FONT_HERSHEY_TRIPLEX, 1, ldm_color, 1)  
                    continue
            cv2.circle(img, (x_pos, y_pos), 5, ldm_color, -1)
            cv2.putText(img, ldm_name_list[idx], (x_pos+10, y_pos-10), cv2.FONT_HERSHEY_TRIPLEX, 1, ldm_color, 1)    
    ...
    def calculate_angle_vector(pointA, pointB, pointC):
        vectorBA = np.array(pointA) - np.array(pointB)
        vectorBC = np.array(pointC) - np.array(pointB)

        # 计算叉积
        cross_product = np.cross(vectorBA, vectorBC)

        # 判断叉积的正负确定方向
        if cross_product > 0:
            angle_rad = 2 * np.pi - np.arccos(np.dot(vectorBA, vectorBC) / (np.linalg.norm(vectorBA) * np.linalg.norm(vectorBC)))
        else:
            angle_rad = np.arccos(np.dot(vectorBA, vectorBC) / (np.linalg.norm(vectorBA) * np.linalg.norm(vectorBC)))
    ...
    def DoubleArrowedLine(img, start, end, eval_color, pixelspacing):
        ...
        if start[1] == end[1]:
            if end[0] == 0:
                ...
                cv2.line(img, (end[1], end[0]), (end[1]-10, end[0]+10), eval_color, thickness=2)
                cv2.line(img, (end[1], end[0]), (end[1]+10, end[0]+10), eval_color, thickness=2)
                cv2.putText(img, "{:.2f}mm".format((start[0]-end[0])*pixelspacing), ( int((start[1]+end[1])/2), int((start[0]+end[0])/2) ), cv2.FONT_HERSHEY_TRIPLEX, 1, eval_color, 1)
            else:
                # down
                cv2.line(img, (end[1], end[0]), (end[1]-10, end[0]-10), eval_color, thickness=2)
                cv2.line(img, (end[1], end[0]), (end[1]+10, end[0]-10), eval_color, thickness=2)
                cv2.putText(img, "{:.2f}mm".format((end[0]-start[0])*pixelspacing), ( int((start[1]+end[1])/2), int((start[0]+end[0])/2) ), cv2.FONT_HERSHEY_TRIPLEX, 1, eval_color, 1)
        if start[0] == end[0]:
            if end[1] == 0:
                # left
                cv2.line(img, (end[1], end[0]), (end[1]+10, end[0]-10), eval_color, thickness=2)
                cv2.line(img, (end[1], end[0]), (end[1]+10, end[0]+10), eval_color, thickness=2)
                cv2.putText(img, "{:.2f}mm".format((start[1]-end[1])*pixelspacing), ( int((start[1]+end[1])/2), int((start[0]+end[0])/2)-10 ), cv2.FONT_HERSHEY_TRIPLEX, 1, eval_color, 1)
            else:
                # right
                cv2.line(img, (end[1], end[0]), (end[1]-10, end[0]-10), eval_color, thickness=2)
                cv2.line(img, (end[1], end[0]), (end[1]-10, end[0]+10), eval_color, thickness=2)
                cv2.putText(img, "{:.2f}mm".format((end[1]-start[1])*pixelspacing), ( int((start[1]+end[1])/2), int((start[0]+end[0])/2)-10 ), cv2.FONT_HERSHEY_TRIPLEX, 1, eval_color, 1)

    eval_color = (125, 129, 254)
    eval_color2 = (223, 184, 129)
    eval_color3 = (255, 255, 77)
    circle_color = (66, 255, 255)
    ...
    if len(landmarks) == 3:
        ...
        # line of [p2 to p3]
        cv2.line(img, (p2[1], p2[0]), (p3[1], p3[0]), eval_color2, thickness=1)
        DoubleArrowedLine(img, ap_midpoint, (0, ap_midpoint[1]), eval_color, pixelspacing=y_pixelspacing) # from midpoint to top
        DoubleArrowedLine(img, ap_midpoint, (img.shape[0], ap_midpoint[1]), eval_color, pixelspacing=y_pixelspacing) # from midpoint to bottom
        DoubleArrowedLine(img, ap_midpoint, (ap_midpoint[0], 0), eval_color, pixelspacing=x_pixelspacing) # from midpoint to left
        DoubleArrowedLine(img, ap_midpoint, (ap_midpoint[0], img.shape[1]), eval_color, pixelspacing=x_pixelspacing) # from midpoint to right
        # X-cross from midpoint
        for point in[(ap_midpoint[1]+10, ap_midpoint[0]+10), 
                     (ap_midpoint[1]-10, ap_midpoint[0]+10), 
                     (ap_midpoint[1]-10, ap_midpoint[0]-10), 
                     (ap_midpoint[1]+10, ap_midpoint[0]-10)]:
            cv2.line(img, (ap_midpoint[1], ap_midpoint[0]), point, eval_color, thickness=2) # bottom right
            cv2.line(img, (ap_midpoint[1], ap_midpoint[0]), point, eval_color, thickness=2) # bottom left
            cv2.line(img, (ap_midpoint[1], ap_midpoint[0]), point, eval_color, thickness=2) # top left
            cv2.line(img, (ap_midpoint[1], ap_midpoint[0]), point, eval_color, thickness=2) # top right
        ...
        if img.shape[0] >= img.shape[1]:
            if p2[1] < p3[1]:
                ...
                l, r = p3, p2
            DoubleArrowedLine(img, l, (l[0], 0), eval_color2, pixelspacing=x_pixelspacing) # left to edge
            cv2.line(img, (l[1], l[0]), (l[1]-10, l[0]+10), eval_color2, thickness=2) # left back to 'l'
            cv2.line(img, (l[1], l[0]), (l[1]-10, l[0]-10), eval_color2, thickness=2) # left back to 'l'
            DoubleArrowedLine(img, r, (r[0], img.shape[1]), eval_color2, pixelspacing=x_pixelspacing) # right to edge
            cv2.line(img, (r[1], r[0]), (r[1]+10, r[0]+10), eval_color2, thickness=2) # right back to 'r'
            cv2.line(img, (r[1], r[0]), (r[1]+10, r[0]-10), eval_color2, thickness=2) # right back to 'r'
            ...
            if r[0] > ap_midpoint[0]: # The right point is below the midpoint in cv2-coordinate-system
                ...
                start_angle = -90
                end_angle = -calculate_angle(r, ap_midpoint, (ap_midpoint[0], img.shape[1])) # cal angle: r-midpoint-right
            cv2.ellipse(img, (ap_midpoint[1], ap_midpoint[0]), (50, 50), 0, start_angle, end_angle, circle_color)
            cv2.putText(img, "{:d} deg".format(int(abs(end_angle-start_angle))), ( ap_midpoint[1]-50, ap_midpoint[0]-50 ), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, circle_color, 1) # write angle
        ...
        if img.shape[0] < img.shape[1]:
            if p2[0] < p3[0]:
                ...
                t, b = p3, p2
            DoubleArrowedLine(img, t, (0, t[1]), eval_color2, pixelspacing=y_pixelspacing) # up to top
            cv2.line(img, (t[1], t[0]), (t[1]+10, t[0]-10), eval_color2, thickness=2) # top back to 't'
            cv2.line(img, (t[1], t[0]), (t[1]-10, t[0]-10), eval_color2, thickness=2) # top back to 't'
            DoubleArrowedLine(img, b, (img.shape[0], b[1]), eval_color2, pixelspacing=y_pixelspacing) # down to bottom
            cv2.line(img, (b[1], b[0]), (b[1]-10, b[0]+10), eval_color2, thickness=2) # down back to 'b'
            cv2.line(img, (b[1], b[0]), (b[1]+10, b[0]+10), eval_color2, thickness=2) # down back to 'b'

            if t[1] > ap_midpoint[1]: # The top point is to the right of the midpoint in cv2-coordinate-system
                start_angle = -calculate_angle(t, ap_midpoint, (ap_midpoint[0], img.shape[1])) # cal angle: r-midpoint-right
                end_angle = 0
            else:
                start_angle = -180
                end_angle = -calculate_angle(t, ap_midpoint, (ap_midpoint[0], img.shape[1])) # cal angle: r-midpoint-right
            cv2.ellipse(img, (ap_midpoint[1], ap_midpoint[0]), (50, 50), 0, start_angle, end_angle, circle_color)
            cv2.putText(img, "{:d} deg".format(int(abs(end_angle-start_angle))), ( ap_midpoint[1]+50, ap_midpoint[0]-50 ), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, circle_color, 1) # write angle
    ...
    if len(landmarks)==5:
        ...
        for point in[(p1[1]+10, p1[0]+10), 
                     ...
                     (p1[1]-10, p1[0]-10), 
                     (p1[1]+10, p1[0]-10)]:
            cv2.line(img, (p1[1], p1[0]), point, eval_color, thickness=2) # bottom right
            cv2.line(img, (p1[1], p1[0]), point, eval_color, thickness=2) # bottom left
            cv2.line(img, (p1[1], p1[0]), point, eval_color, thickness=2) # top left
            cv2.line(img, (p1[1], p1[0]), point, eval_color, thickness=2) # top right

        cv2.line(img, (p1[1], p1[0]), (lat_proximal[1], lat_proximal[0]), eval_color2, thickness=2) # from p1 to lat_proximal
        cv2.line(img, (p1[1], p1[0]), (p2[1], p2[0]), eval_color3, thickness=2) # from p1 to 'center of remote Radius'
        cv2.line(img, (p1[1], p1[0]), (p4[1], p4[0]), eval_color3, thickness=2) # from p1 to 'center of remote Ulna'
        ...
        if lat_proximal[0] > p1[0]: # lat_proximal is below p1
            ...
            end_angle = 90
            cv2.ellipse(img, (p1[1], p1[0]), (50, 50), 0, start_angle, end_angle, circle_color, 1)
        ...
        if abs(angle1 - angle2) < 180:
            ...
            bigger, smaller = max(angle1, angle2), min(angle1, angle2)
            cv2.ellipse(img, (p1[1], p1[0]), (20, 20), 0, bigger-360, smaller, eval_color3, 1)
            cv2.putText(img, "{:d} deg".format(360-int(abs(angle1-angle2))), ( p1[1]-50, p1[0]+50 ), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, eval_color3, 1) # write angle 
        cv2.circle(img, (p1[1], p1[0]), 5, (255, 255, 255), -1)
    return img
    ...
    for idx, (y_pos, x_pos) in enumerate(landmarks):
        ...
        if len(landmarks)==5 and (idx==2 or idx==4):
            ...
            if idx == 4:
                ...
                cv2.putText(img, ldm_name_list[idx], (lat_proximal[1]+10, lat_proximal[0]-10), cv2.FONT_HERSHEY_TRIPLEX, 1, ldm_color_list[idx], 1)  
                break
        cv2.circle(img, (x_pos, y_pos), 3, ldm_color_list[idx], -1)
        cv2.putText(img, ldm_name_list[idx], (x_pos+10, y_pos-10), cv2.FONT_HERSHEY_TRIPLEX, 1, ldm_color_list[idx], 1)    
